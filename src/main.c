...
void *works_list_for_phils(void *ptr)
{
    ...
    while(1337)
    {
        ...
        if(data->argc == 6)
            if (  data->philo[num_of_philo].num_of_times_eat >= data->number_of_times_each_philosopher_must_eat)
                return (NULL);
...
int ft_create_thread(t_data *data)
{
    ...
    while(++data->a < data->number_of_philosophers)
    {
        if(pthread_create(&data->philo[data->a].phil, NULL , &works_list_for_phils, data) != 0)
...
int ft_data_init(t_data *data, char **argv)
{
    ...
    data->mutex_eat = malloc(sizeof(pthread_mutex_t) * data->number_of_philosophers); 
    if(!data->mutex_eat)
